...
status: active
tags: [abm, account-based, target-accounts, buying-group, intent, enterprise, agentic, skills]
domain: skills
parent: skills/index
---
...
ABM, account-based marketing, is the skill of running a small number of high-value accounts as coordinated programs instead of as leads in a funnel. You run it when a named account is worth more than the sum of the contacts you can reach there, and when the deal economics justify the attention.

The unit of ABM is the account, not the lead. Every touch, from ads to email to a sales call, coordinates around the account's buying group and its specific path to a decision. The reason to run ABM is concentration: when a handful of accounts can move your quarter, you treat each one like a campaign.
...
ABM is a coordinated, account-typed program that treats each target account as a market of one. It combines account selection, intent and account planning, a coordinated set of touches across sales and marketing, and a measurement loop that reads the account's progress toward a deal, not the count of leads.
...
### 1. Pick the accounts that earn ABM
...
One claim, many surfaces. The account should see the same value hypothesis on your site, in the ad, in the email, and on the call.

### 5. Measure account progress, not lead count

The ABM metric is the account's movement toward a deal, not the number of leads it produced. Track per account:

- **Engagement.** Which members of the buying group are active and on which pages.
...
One page per Tier 1 account.

```text
Account:             <name>
ICP fit (0-5):       <fit score>
Expected value:      <ACV and expansion>
...
- Ad / web:      <intent-based ad + personalized claim>
- Email:         <signal-led touch naming the account's situation>
- Sales:         <account plan in the rep's hands>
- Event / asset: <the specific piece that makes the internal case>
```

## Worked example

A B2B SaaS sells a revenue-operations platform to mid-market and enterprise accounts at roughly $120K ACV. It selects 60 accounts by fit, value, and intent: 8 Tier 1, 24 Tier 2, 28 Tier 3.
...
An agent can select, score, and route accounts, execute Tier 2 and Tier 3 touches, and assemble the account plan from intent data, but it cannot decide the value hypothesis or the buying-group strategy. Give it the tiering rules, the intent feed, and the template; it will do the execution and the measurement. Keep the account strategy and the Tier 1 relationship with you.

**SOP: execute an ABM tier.**

```text
Given: a tiered account list, intent data, a value hypothesis per account, and a claim.
1. Re-score each account on fit, value, and current intent.
2. For Tier 1: draft the account plan and buying-group map; route to a human for sign-off.
3. For Tier 2/3: queue intent-based ads and web personalization with the shared claim.
4. Sequence signal-led email to the buying group; fill the variable block with the account's signal.
5. Log engagement per buying-group member and pipeline movement per account.
6. Compare ABM vs non-ABM win rate and deal size at the end of the period.
Never invent a value hypothesis or a proof point. Never run a Tier 1 account without a human-signed plan.
```

**Prompt: draft the account plan draft.**

```text
You are an ABM operator. Given an account, its intent signals, and the ICP,
draft a one-page account plan using the template.
Name the value hypothesis, the buying group roles, the competitive frame, the
key objection, and the proof. Keep it to one page. No superlatives.
Output the plan only.
```

Keep the human in the loop for the value hypothesis, the buying-group strategy, and any Tier 1 executive touch. The agent assembles and executes; you decide the account bet.

## Run it today

Score your current target list on fit, value, and intent, and split it into three tiers. Write one Tier 1 account plan and map the buying group. Pick the claim from your positioning foundation and run it consistently across the ad, the email, and the call for that one account. The first coordinated account is the proof the program works.

## Sources

1. [Amplitude: buying committees](https://amplitude.com): ABM buying committees average 7-8 people; enterprise committees reach 11-17.
2. [Foundry: intent data](https://foundryco.com): intent-based ads show higher click-through; 71% of B2B teams use intent data.
3. [Demandbase 2026: buying groups](https://demandbase.com): engaging buying groups drives 2-3x higher win rates and larger deals.
4. [SalesMotion 2026: ABM win rates](https://salesmotion.com): ABM win rate 45% vs 18% non-ABM; ~21% all, ~29% qualified, ~22% SaaS.
5. [TOPO 2019: ABM benchmark](https://topohq.com): 86% of organizations attribute increased win rate to ABM.
6. [SiriusDecisions / Forrester: defined ICP](https://www.siriusdecisions.com): a defined ICP drives 68% higher win rates and 30% more marketing-sourced revenue.
7. [Spiceworks: ABM account counts](https://www.spiceworks.com): 57% of B2B teams target 1,000 or fewer ABM accounts.
8. [Demand Gen Report 2020: ABM focus](https://www.demandgenreport.com): 29% of teams focus on 101-500 accounts.
9. [Insight Collective: ABM tiers](https://insightcollective.com): one-to-one is a handful of accounts, one-to-few is clusters of 20-100, one-to-many is hundreds to thousands.
10. [Autobound 2026: signal personalization](https://autobound.ai): multi-channel reply rates of 15-25% from signal-based outreach.
11. [Positioning and messaging foundation](../foundations/03-positioning-and-messaging.md): the claim ABM coordinates across every surface.
