...
status: active
tags: [seo, organic, search, content, ranking, keyword, agentic, skills]
domain: skills
parent: skills/index
---
...
- You have a content or search program with a horizon longer than one quarter.
- Your buyers search for the problem you solve before they buy.
...
SEO is the repeatable process of matching pages to search demand and making each page the best result for its query. It combines topic selection, on-page and technical execution, content quality, and a measurement loop that reads rankings, clicks, and conversions. The page that ranks is the output; the traffic and pipeline it earns is the result.
...
### 1. Pick the queries worth winning

Start from search demand your buyers actually have, tied to your [messaging and positioning](./messaging). Build a topic map:

- **Head terms.** Broad, high-volume queries for your category. Hard to win, high value.
- **Long-tail terms.** Specific, lower-volume queries that match a buyer's exact problem. Easier to win, higher conversion.
- **Problem terms.** The pain your buyers name before they know your solution exists.

Score each candidate query on demand, relevance, and winnability. A query you cannot win in the year is a query to leave to later; a query your page already answers better than the incumbents is a query to build now.

### 2. Build the page that is the best answer
...
A great page still needs the technical foundation to be indexed and understood:

- **Crawlable.** Internal links reach it; the sitemap lists it; nothing blocks the crawler.
- **Fast.** Core Web Vitals pass on mobile and desktop.
- **Structured.** One clear H1, a descriptive title and meta description, and headings that reflect the content.
- **Correct URL.** A clean, permanent URL that does not change as the site evolves.

Technical SEO does not rank a thin page, but it lets a good one rank.

### 4. Win the result, then hold it
...
Score each candidate query before you build. Build the winnable ones first.

```text
Query                    | Intent     | Demand | Relevance | Winnable | Build
-------------------------|------------|--------|-----------|----------|------
"gtm metrics benchmarks" | benchmark  | high   | high      | no (hub) | later
"cold email reply rate"  | data       | med    | high      | yes      | now
"cac payback by segment" | data       | med    | high      | yes      | now
"what is abm"            | definition | high   | med       | no       | later
```

Threshold to build now: demand at least med, relevance high, winnable yes.
...
An agent can build the topic map, draft the page, check it against the quality gate, and log the measurement loop, but it cannot decide the topic strategy or the claim. Give it the query list, the messaging foundation, and the quality bar; it will draft and measure. Keep the topic selection and the best-answer judgment with you.

**SOP: build and maintain a page.**
...
Never assert a stat without a named, dated source. Never publish a page below the quality bar.
```

**Prompt: draft a benchmark page.**

```text
You are a GTM content operator. Given a query and the messaging foundation,
write a page that is the best answer to the query.
Match the page to the query's intent. Use named, dated stats with sources.
Show a worked example. No superlatives, no hype, active voice.
Output the page only.
```

Keep the human in the loop for the topic strategy, the claim, and the final quality pass. The agent drafts and measures; you decide what to build.

## Run it today

Build a topic map of 20 queries your buyers search, score each on demand, relevance, and winnability, and pick the 5 you can win this quarter. Write the first as the best answer with named, dated sources and a worked example. The first ranking page is the proof the channel works.

## Sources

1. [DigitalApplied 2026: CAC by channel](https://www.digitalapplied.com): organic acquisition CAC is a fraction of paid; organic costs $348 per customer vs $1,180 paid search, and the asset persists.
2. [DemandSage 2026: content marketing](https://www.demandsage.com): content generates about 3x more leads than outbound and costs 62% less.
3. [Ahrefs 2026: ranking odds](https://ahrefs.com): fewer than 2% of new pages reach Google's top 10 within a year; ~5% do with at least one backlink; 96.6% of clicks go to first-page results.
4. [Seer Interactive 2025: AI overviews](https://seerinteractive.com): click-through fell roughly 61% on queries where AI Overviews appear.
5. Grokipedia-Exceeding Standard (GE Standard): the depth, breadth, citation, and artifact bar any content page should clear.
6. [Positioning and messaging foundation](../foundations/03-positioning-and-messaging.md): lead with the problem, use the buyer's language, proof over claims.
