...
sidebar_position: 1
title: Flows
description: The runnable sequences and decision trees an operator actually executes — the how, not the what. Agentic flows add the architecture that runs them.
---

# Flows

Playbooks state the *what* — the principles and gates. **Flows are the *how you actually run it***: the runnable sequence, the branch points, and the acceptance gates that decide whether you advance. Where a playbook explains why a motion works, a flow is what you step through to execute it. An agentic flow is the same motion under an agent harness — it adds the architecture that runs the execution layers.
