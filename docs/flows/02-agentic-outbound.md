...
This is the executable version of [Agentic Outbound](/docs/agentic/agentic-outbound) and the running twin of the [Outbound from Zero](/docs/playbooks/outbound-from-zero) playbook. The manual flow is the sequence an operator steps through by hand. This flow is the same sequence when an agent use runs the execution layers.

The gates do not move. Every row still needs a first name, a verified email, and a one-line reason-to-reach. Every variant still earns human approval before the first send. Nothing automates until 300+ sends prove a pattern. What changes is who sources, who drafts, who sends, and who answers replies. The sequence stays runnable, with an acceptance gate at every step. **Do not advance on a miss.**

Measure the run against the [Outbound Response Rate Benchmarks](/docs/data/outbound-response-rates). Judge the channel choice against the [Outbound channel](/docs/channels/outbound) page.
...
The flow runs three stages, in order. Each stage has an acceptance gate that is a hard stop until it passes.

1. **Prove the message.** Small list, high-touch, all human judgment. Find the offer that gets replies. Nothing is automated until the manual version beats baseline. *Gate: 300+ sends per variant and a positive-reply signal worth scaling.*
2. **Automate the use.** Agents source, draft, and route at scale. Humans approve the pattern, not every keystroke. *Gate: brief coverage above 90% so the "specific" lines are grounded.*
3. **Guard the quality bar.** Harness-health metrics are non-negotiable. If any slips, the loop is scaling garbage. Shrink it. *Gate: triage accuracy above 95%, override rate under 5%, hot replies answered inside 15 minutes.*

The layer stack the flow runs on is the [Agentic Stack](/docs/agentic/agentic-stack), placed on the autonomy ladder from [Agent vs Human Workflows](/docs/agentic/agent-vs-human-workflows), and held honest by [Guardrails and Measurement](/docs/agentic/guardrails-and-measurement).
...
The harness builds on a proven motion, never a substitute for proving it. Stage 1 is identical to the manual flow in [Outbound from Zero](/docs/playbooks/outbound-from-zero).

**Acceptance gate for Stage 1:** you have 300+ sends per variant, a delivery rate above 97%, and a positive-reply signal worth scaling. The winning variant is kept, and you can name the metric that was failing before you changed anything.

Do not optimize a message on a smaller sample. Small samples produce noise, and noise produces confident wrong answers.

---

## Stage 2: Automate the use (the sequence in detail)

Once the message is proven, the use runs each layer. This section is the runnable detail of each step, with its own gate.
...
Delivery is the plumbing, and the plumbing is where campaigns quietly die. The orchestrator handles cadence, warmup, domain rotation, and sending caps. None of that is optional.
...
**Acceptance gate for 2.4:** hot replies are drafted and surfaced within 5 minutes of arrival, every hot reply is human-approved before send, and the 15-minute response window holds. If replies pile up with no SLA, the loop is dying quietly.
...
| Triage accuracy | >95% correct routing | The loop will not bury hot leads |
| Time-to-first-response | Under 15 minutes for hot | The loop converts replies into meetings |
| Override rate | Under 5% | The use is not saying things you would veto |
...
with company, ICP-fit score (1 to 5), primary contact first name,
...
**The one rule that never changes across all three prompts: the gate stays, the execution layer changes.** Humans keep judgment and approval. Agents remove the thinking bottleneck at volume.

---

## Handoff (unchanged, still the failure point)

Agents do not fix a broken handoff. They make it faster. If you do not have a meeting standard, source tracking, and a 2-week debrief rhythm, automation just fills the funnel faster with unqualified meetings.

1. **Define the meeting standard.** Write down who counts as a qualified meeting. If you cannot, "meetings booked" is meaningless.
2. **Track the source.** Every meeting traces to a campaign. No source, no scale.
3. **Debrief every 2 weeks.** What message won, what list segment converted, what objection killed the deal.
4. **Feed the win back into the list.** Your best customers become your new look-alike list. The motion is a loop.

**Acceptance gate for handoff:** the meeting standard is written, every meeting traces to a campaign, and the 2-week debrief rhythm exists.

---

## Worked example: one agentic campaign end to end

A representative build, step by step, so you can see the whole thing hold together. Names and figures are illustrative but plausible.
...
Measure use health -- triage >95% & override under 5% & delivery >97%? -- no -- shrink it
  | yes
Scale + handoff (unchanged: automation does not fix a broken handoff)
```

Every branch that routes sideways is a gate failing. The flow does not continue until the gate passes.
...
- [Agentic Outbound](/docs/agentic/agentic-outbound): the execution-layer source for the augment-don't-replace case and the 2.8x and 15-versus-25% numbers.
- [Outbound from Zero](/docs/playbooks/outbound-from-zero): the manual playbook this harness accelerates, with the full copy and deliverability setup.
- [Agentic Stack](/docs/agentic/agentic-stack): the five-layer architecture this flow runs on.
- [Agent vs Human Workflows](/docs/agentic/agent-vs-human-workflows): the autonomy ladder and human-in-the-loop controls.
- [Guardrails and Measurement](/docs/agentic/guardrails-and-measurement): the quality bars and when to turn the use off.
- [Outbound Response Rate Benchmarks](/docs/data/outbound-response-rates): the numbers this flow is measured against.
- [Outbound channel](/docs/channels/outbound): when outbound is the right call before you commit budget.

---

## Sources

-[1] [Autobound, AI SDR Tools Guide 2026](https://www.autobound.ai/blog/ai-sdr-tools-guide): AI-augmented teams produce 2.8x more pipeline than full-replacement teams; autonomous AI SDR tools churn at 50-70% annually, roughly double human turnover; autonomous agents convert meetings to qualified opportunities at 15% versus 25% for human SDRs. Vendor source via UserGems.

-[2] [UserGems, AI SDR Churn Analysis 2026](https://www.usergems.com/blog/ai-sdr-churn): AI SDR tools churn at 50-70% annually, roughly double the turnover of human reps.

-[3] [Fullcast, The Agentic Stack 2026](https://www.fullcast.io/agentic-stack): Five-layer architecture: data, research, compose, orchestrate, triage. Governance and human oversight with phased implementation.

-[4] [SuperAGI, AI vs Traditional SDRs 2026](https://superagi.com/ai-vs-traditional-sdrs-a-comparative-analysis-of-pipeline-performance-and-cost-efficiency/): Autonomous agents convert meetings to qualified opportunities at 15% versus 25% for human SDRs, a gap driven by weak objection handling, shallow relationship building, and degraded judgment at scale.

-[5] [Clay, Outbound Sales Automation 2026](https://www.clay.com/blog/outbound-sales-automation): Research brief quality determines message quality; a thin research brief poisons everything downstream.

-[6] [Amplemarket, Best AI Sales Agents 2026](https://www.amplemarket.com/blog/best-ai-sales-agents): AI agents assemble per-prospect research briefs a human would spend 15-30 minutes building by hand; tone exemplars train the compose layer.

-[7] [Smartlead, AI Agents for Outbound Sales 2026](https://www.smartlead.ai/blog/ai-agents-for-outbound-sales): Signal-triggered outreach reaches 14-25% reply rates against a 3.43% cold-email industry average.

-[8] [Mailforge, B2B Cold Email Statistics 2026](https://martal.ca/b2b-cold-email-statistics-lb/): 50-125 word emails earn roughly 50% higher reply rates than longer formats.

-[9] [Martal, B2B Cold Email Statistics 2026](https://martal.ca/b2b-cold-email-statistics-lb/): Specific CTAs that name the issue outperform generic "let's talk" approaches.

-[10] [Instantly, 2025 Guide to AI Outbound Sales](https://instantly.ai/blog/2025-guide-to-ai-outbound-sales/): Reply agents classify intent and draft responses in under 5 minutes; signal-based personalization reaches 15-25% reply rates.

-[11] [Instantly, Cold Email Benchmark Report 2026](https://instantly.ai/cold-email-benchmark-report-2026): Signal-based personalization reaches 15-25% reply rates versus 3-5% single-channel baseline; hot replies must be handled within 15 minutes.

-[12] [Mailshake, State of Cold Email 2025](https://mailshake.com/blog/the-state-of-cold-email-2025/): Only 5% of senders personalize every email.

-[13] [Mailshake, State of Cold Email 2025](https://mailshake.com/blog/the-state-of-cold-email-2025/): Senders who personalize every email see 2-3x better results.

-[14] [Infraforge, B2B Cold Email Statistics 2026](https://martal.ca/b2b-cold-email-statistics-lb/): Advanced personalization (specific, signal-driven references) lifts replies to 18% versus about 9% for generic templates.

-[15] [Smartlead, AI Agents for Outbound Sales 2026](https://www.smartlead.ai/blog/ai-agents-for-outbound-sales): Fully autonomous systems at scale run 1-3% reply rates; hybrid setups run 8-15%. Volume on shared infrastructure poisons domain reputation; one customer saw reply rates fall from 6% to under 2%.

-[16] [Woodpecker, Cold Email Statistics 2026](https://woodpecker.co/blog/cold-email-statistics/): Cold email reply average 3.43%; good to excellent ranges from 5% to 10% or higher.

-[17] [SalesHive, B2B Cold Email Statistics 2025](https://saleshive.com/blog/b2b-cold-email-statistics/): Blended B2B cold reply rate of 5.8%; positive reply rate above 30% of total replies.
...
1. The 2.8x pipeline figure compares hybrid human-in-the-loop teams with teams attempting full replacement of their SDRs. Both figures come from vendor sources that describe what is possible, not what is typical. Independent verification is recommended before budgeting.
2. The 50–70% churn rate applies to fully autonomous AI SDR tools. Hybrid teams that keep human oversight on sends show materially lower churn, as noted in the augment-don't-replace section above.
3. The layer stack (data, research, compose, orchestrate, triage) is one framing of agentic GTM architecture. Other vendors may describe different but functionally equivalent layers.
4. The 15% versus 25% conversion gap applies to autonomous agents versus human SDRs. The gap narrows when human oversight is maintained on the compose and triage layers, as this page recommends.
